...
namespace ProductCatalogApi.Data
{
    public class CatalogSeed
    {
        public static async Task SeedAsync(CatalogContext context)
        {
            ...
            {
                context.CatalogBrands.AttachRange(PreconfiguredCatalogBrands);
                await context.SaveChangesAsync();
            }

            if (!context.CatalogTypes.Any())
            {
                context.CatalogTypes.AttachRange(PreconfiguredCatalogTypes);
                await context.SaveChangesAsync();
            }

            if (!context.CatalogItems.Any())
            {
                context.CatalogItems.AttachRange(PreconfiguredItems);
                await context.SaveChangesAsync();
            }
        }

        private static IEnumerable<CatalogBrand> PreconfiguredCatalogBrands => new List<CatalogBrand>
            {
        ...
        private static IEnumerable<CatalogType> PreconfiguredCatalogTypes => new List<CatalogType>
            {
        ...
        private static IEnumerable<CatalogItem> PreconfiguredItems => new List<CatalogItem>
            {
